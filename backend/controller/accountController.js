...
const getAllInforAcc = async (req, res) => {
    const sql = "SELECT a.username, a.role, a.status, m.name AS member_name, m.address, m.phone_number, m.email, p.name AS position_name FROM tbl_account a JOIN tbl_member m ON a.username = m.account_id JOIN tbl_position p ON m.position_id = p.id;";
    db.query(sql, (err, result) => {
      if (err) {
        return res.status(500).json({ Error: "Error fetching account" });
      }
...
const updateaccountusername = async (req, res) => {
    ...
    // Only append the WHERE clause if any fields were provided
    sql += " WHERE username = ?";
